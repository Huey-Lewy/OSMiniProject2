...
def _append_sched_block(ts: int, procs: List[Proc]) -> None:
    """Append a complete SCHED_LOG block (format matches agent_bridge.py)."""
...
def _tail_for_advice(ts: int, start_offset: int, timeout: float) -> Optional[int]:
    """Tail llm_advice.txt for a matching TS=<ts> advice line."""
...
def run_case(tc: TestCase) -> bool:
    """Append log, wait for advice, compare agent and expected PIDs."""
