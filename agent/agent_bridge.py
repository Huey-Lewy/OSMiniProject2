...
class LLMSchedulerAgent:
  ...
  def _read_log(self) -> Optional[Tuple[int, List[ProcessStats]]]:
    ...
    try:
        ...
        if line.startswith("TIMESTAMP:"):
          ...
          if len(p) == 6:
            try:
              ...
              # Skip malformed lines instead of failing the whole block.
              continue

      # Require both a timestamp and at least one process to be considered valid.
      ...
      # The scheduler has not created the log yet; keep polling.
  ...
  def _runnable(self, procs: List[ProcessStats]) -> List[ProcessStats]:
    ...
    For xv6:
        state == 3  → RUNNABLE
        pid   > 2   → skip init / shell / very early system processes
    """
  ...
  def _make_prompt(self, procs: List[ProcessStats]) -> Optional[str]:
    ...
    if len(ready) > MAX_PROCS_IN_PROMPT:
      # Sort by highest wait time and IO count, then trim.
    ...
    # Attach one line per runnable process with its key metrics.
    for p in ready:
      lines.append(
  ...
  def _ask_llm(self, prompt: str) -> Optional[int]:
    ...
    try:
      ...
      # Extract the "response" field and try to find "PID:<number>".
      txt = (r.json().get("response") or "").strip()
      m = re.search(r"PID[:\s]+(\d+)", txt, flags=re.IGNORECASE)
      if not m:
        # Fallback: if the model ignored the format, grab the first integer we see.
  ...
  def _choose_with_retry(self, prompt: str, runnable_pids: set[int]) -> Optional[int]:
    ...
    The LLM must choose a PID that exists in the current snapshot's
    runnable_pids. If it chooses something else or we cannot parse the
    response, we retry (with a short sleep between tries).

    Returns:
        int or None: Chosen PID, or None if all attempts fail.
    """
    tries = RETRIES + 1
    for attempt in range(1, tries + 1):
      ...
      # Sleep between retries unless this was the last attempt.
  ...
  def _fallback_choice(self, procs: List[ProcessStats]) -> Optional[int]:
    ...
    Scoring heuristic:
      - Favor higher WAIT and IO (bigger is better).
      - Penalize higher RECENT CPU (avoid hogs).
      - Use CPU ticks as a secondary tiebreaker (lower total CPU first).

    Returns:
        int or None: Chosen PID, or None if nothing runnable.
    """
    ready = self._runnable(procs)
    if not ready:
      return None
    ...
    def score(p: ProcessStats) -> Tuple[float, int]:
      # Weighted score: WAIT + IO - RECENT, plus tiny jitter by PID to break ties.
  ...
  def _write_fifo(self, line: str) -> None:
    ...
    Behavior:
      - If the FIFO does not exist, this does nothing.
      - If there is no reader yet, open() fails with ENXIO; this is ignored.
      - On success, the line is written and the descriptor is closed.
    """
    try:
      ...
      # No reader or FIFO not ready; skip quietly.
  ...
  def _write(self, pid: int, ts: int):
    """
    Append an advice line to the advice file, guarded against duplicates.
    ...
    The line is written both to an append-only log file and (best-effort)
    to a FIFO for live injection into xv6 via console_mux.py.
    """
    try:
  ...
  def last_log_size(self) -> int:
    """Expose the current log cursor position (bytes read)."""
    return self.last_size

  @last_log_size.setter
  def last_log_size(self, v: int) -> None:
    """Allow tests to reset or override the log cursor position."""
    try:
  ...
  def read_scheduling_log(self):
    """Public wrapper around _read_log() for testing."""
    return self._read_log()

  def format_prompt_for_llm(self, processes):
    """Public wrapper around _make_prompt() for testing."""
    return self._make_prompt(processes)

  def query_llm(self, prompt: str):
    """Public wrapper around _ask_llm() for testing."""
    return self._ask_llm(prompt)

  # Optional back-compat (if any external callers still use these names).
  ...
  def run(self):
    ...
    Steps:
      - Read any new scheduler log data since last_size.
      - Extract the latest complete snapshot (timestamp + processes).
      - If there are runnable processes:
          * If exactly one → write advice directly (no LLM call).
          * If more than one → ask LLM (with retries) or use fallback policy.
      - Append ADVICE line to advice file and mirror to FIFO.
    """
    ...
    while self.running:
      ...
      if parsed:
        ...
        # Only generate one advice per unique timestamp.
        ...
        # Fast path: nothing runnable in this snapshot → idle.
        if len(runnable) == 0:
          time.sleep(self.interval)
          continue

        # Fast path: exactly one runnable process → no need to ask the LLM.
        if len(runnable) == 1:
          self._write(runnable[0].pid, log_ts)
          time.sleep(self.interval)
          continue

        # General case: multiple runnable processes.
        ...
        if prompt:
          ...
          if pid is not None:
            ...
            # After exhausting retries, fall back to deterministic scoring.
            print(f"[agent] All retries failed @TS={log_ts}; using fallback.")
            chosen_pid = self._fallback_choice(processes)
        else:
          # If prompt construction failed, we still try the fallback policy.
          chosen_pid = self._fallback_choice(processes)

        # If we have a chosen PID, write advice for this timestamp.
        if chosen_pid is not None:
          self._write(chosen_pid, log_ts)

      # Sleep between polls to avoid busy waiting on the log file.
      time.sleep(self.interval)
